...
void analyzer::Driver::parse_helper(std::istream &i_stream, std::ofstream &o_stream) {
    ast::PGoal root;

    delete(scanner);
    scanner = new analyer::Scanner(&i_stream);

    delete(parser);
    parser = new analyzer::Parser(*scanner, *this, &root);
    const int accept = 0;
